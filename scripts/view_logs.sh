...
list_logs() {
    print_status "Available log files for $SYSTEM_ID:"
    echo
...
view_latest() {
    local latest_log=$(find "$SYSTEM_DIR" -name "*.log" -type f | sort -r | head -n 1)
    if [ -n "$latest_log" ]; then
        view_log "$latest_log"
    else
        print_warning "No log files found for $SYSTEM_ID"
    fi
}
...
view_today() {
    ...
    local today_logs=$(find "$SYSTEM_DIR" -name "${today}-*.log" -type f | sort -r)
    if [ -n "$today_logs" ]; then
        print_status "Today's log files for $SYSTEM_ID:"
        echo "$today_logs" | while read -r logfile; do
            view_log "$logfile"
            echo
        done
    else
        print_warning "No log files found for today ($today) in $SYSTEM_ID"
    fi
}
...
search_logs() {
    ...
    print_status "Searching for '$search_term' in $SYSTEM_ID log files..."
    find "$SYSTEM_DIR" -name "*.log" -type f -exec grep -l "$search_term" {} \; | while read -r logfile; do
        filename=$(basename "$logfile")
        filedir=$(dirname "$logfile")
        month_dir=$(basename "$filedir")
